...
const testNotifications = [
  ...
  { id: 4, type: 'urgent', isRead: false, value: 'Complete React project by Friday' },
];
...
test('clicking urgent filter button shows only urgent notifications', () => {
  ...
  fireEvent.click(screen.getByTestId('filter-urgent'));
...
test('clicking default filter button shows only default notifications', () => {
  ...
  fireEvent.click(screen.getByTestId('filter-default'));

  const items = screen.getAllByRole('listitem');
  expect(items).toHaveLength(1);
  expect(screen.getByText(/new course available/i)).toBeInTheDocument();
  expect(screen.queryByText(/new resume available/i)).not.toBeInTheDocument();
});

test('clicking urgent filter twice resets to show all notifications', () => {
  render(<Notifications notifications={testNotifications} />);
